...
void Test_Programer()
{
    ...
    std::set<Programmer, ProgrammerIdGreater<Programmer>> ProgSet(OriginData,OriginData+6);
    ...
    //4.3. 使用static_cast<Programmer&>(*it)来试试
    std::wcout << L"\n4.4. Modify set element Use const_cast<Programmer&>(*it):\n";
    std::set<Programmer, ProgrammerNameComparer<Programmer>>::iterator
        it = std::find_if(ProgSet.cbegin(), ProgSet.cend(), FindProgramer(objOrigin));
    if (it != ProgSet.cend()) {
        const_cast<Programmer&>(*it).setName(newName);
    }

    std::for_each(ProgSet.cbegin(), ProgSet.cend(), PrintObj);
    std::cout << "\nOK!!The name of 'Bill Gates' was modified by const_cast<Programmer&> successfully!\n";
    //5.重新定义一个新set，并且按Name排序（升序）
    std::wcout << L"\n5.make a new set set2\n";

    std::cout << "\n5.1.make set2 ordered by Name ASC;\n";
    std::set<Programmer, ProgrammerNameComparer<Programmer>> set2(OriginData,OriginData+6);
